...
import { FavoriteWordDto, LearnedWordDto, AddFavoriteWordDto, AddLearnedWordDto} from '../_models/user-word_models';
...
export class UserWordService {
  ...
  getFavorites(): Observable<FavoriteWordDto[]> {
    return this.http.get<FavoriteWordDto[]>(`${this.apiUrl}/api/userwords/favorites`, {
      headers: this.getAuthHeaders()
    });
  }

  getFavoriteById(id: string): Observable<FavoriteWordDto> {
    return this.http.get<FavoriteWordDto>(`${this.apiUrl}/api/userwords/favorites/${id}`, {
      headers: this.getAuthHeaders()
    });
  }

  addFavorite(dto: AddFavoriteWordDto): Observable<FavoriteWordDto> {
    return this.http.post<FavoriteWordDto>(`${this.apiUrl}/api/userwords/favorites`, dto, {
      headers: this.getAuthHeaders()
    });
  }

  removeFavorite(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/api/userwords/favorites/${id}`, {
      headers: this.getAuthHeaders()
    });
  }
